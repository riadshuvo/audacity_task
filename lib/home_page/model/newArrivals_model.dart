...
List<List<NewArrivalsModel>> newArrivalsModelFromJson(String str) => List<List<NewArrivalsModel>>.from(json.decode(str).map((x) => List<NewArrivalsModel>.from(x.map((x) => NewArrivalsModel.fromJson(x)))));

String newArrivalsModelToJson(List<List<NewArrivalsModel>> data) => json.encode(List<dynamic>.from(data.map((x) => List<dynamic>.from(x.map((x) => x.toJson())))));
